...
export default function DemandGenerationHero() {
  ...
  useEffect(() => {
    ...
    }, 3000); // Slowed to 3s for very stable reading
    return () => clearInterval(interval);
  }, []);
  ...
  return (
    <div className="w-full h-full bg-slate-950 text-white relative overflow-hidden font-sans flex items-center justify-center select-none">
      ...
      {/* Safe Area Container */}
      <div className="w-full h-full relative flex flex-col items-center justify-between z-10 py-4 px-2 sm:px-6">

        {/* --- Header / Top Section --- */}
        <div className={`w-full flex justify-between items-start mb-2 transition-opacity duration-300 ${selectedStage ? 'opacity-30' : 'opacity-100'}`}>
            {/* Left Column */}
        ...
        <div className="flex-1 w-full relative flex items-center justify-center min-h-0">
          ...
          <div className="relative w-[360px] flex flex-col items-center perspective-[1000px] transform-style-3d z-10">
             
             {/* Particles Layer (Constrained to stack height) */}
          ...
          {/* Moved Lead Velocity to top-1/2 to avoid overlap with wide top funnel */}
          <FloatingCard 
        ...
        {/* --- Bottom Section / Animated Metrics Grid --- */}
        <div className={`w-full grid grid-cols-3 gap-2 sm:gap-4 mt-2 transition-opacity duration-300 ${selectedStage ? 'opacity-30' : 'opacity-100'}`}>
